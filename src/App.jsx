...
export default function App() {
  const [auth, setAuth] = useState(null); // { token, user: { id, email, role, first_name, last_name } }
  ...
  const [adminTab, setAdminTab] = useState("courses"); // courses | users | logs
  ...
  async function loadLogs() {
    const data = await api("/api/logs", { token });
    setLogs(data);
  }

  async function loadMyCourses() {
    const data = await api("/api/user-courses/my-courses", { token });
    setMyCourses(data);
  }
  ...
  async function enroll(courseId) {
    try {
      await api("/api/user-courses/enroll", {
        method: "POST",
        token,
  ...
  async function unenroll(courseId) {
    try {
      await api(`/api/user-courses/unenroll/${courseId}`, {
        method: "DELETE",
        token,
      });
      await loadMyCourses();
            ...
            {view === "admin" ? (
              <div style={{ border: "1px solid #ddd", padding: 16, borderRadius: 10 }}>
                ...
                {adminTab === "courses" ? (
                  ...
                  <>
                    ...
                    <div style={{ display: "grid", gap: 8, gridTemplateColumns: "1fr 1fr", maxWidth: 900 }}>
                      ...
                      <input
                        type="date"
                        value={uBirth}
                        onChange={(e) => setUBirth(e.target.value)}
                        style={{ padding: 8 }}
                      />
                      <input
                          ...
                          {editingUserId === u.id ? (
                            <div style={{ display: "grid", gap: 8, gridTemplateColumns: "1fr 1fr" }}>
                              ...
                              <div style={{ gridColumn: "1 / -1" }}>
                                <button onClick={() => saveEditUser(u.id)} style={{ marginRight: 8 }}>Save</button>
                  ...
                  <>
                    <h3 style={{ marginTop: 0 }}>Admin — Logs</h3>
                    <ul style={{ paddingLeft: 18 }}>
                      {logs.map((l) => (
                        <li key={l._id || l.id} style={{ marginBottom: 8 }}>
              ...
              <div style={{ border: "1px solid #ddd", padding: 16, borderRadius: 10 }}>
                ...
                <ul style={{ paddingLeft: 18 }}>
                  {myCourses.map((c) => (
                    <li key={c.id} style={{ marginBottom: 8 }}>
                      <b>{c.name}</b> — {c.type}
                      <button onClick={() => unenroll(c.id)} style={{ marginLeft: 10 }}>
                        Unenroll
                      </button>
                    </li>
                  ))}
